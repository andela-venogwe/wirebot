...
login_to_docker_registry() {
    ...
    sudo docker login -u _json_key --password-stdin https://${DOCKER_REGISTRY} < ${HOME}/gcloud-service-key.json
}

logout_of_docker_registry() {
    echo "====> Logout from docker"
    sudo docker logout https://${DOCKER_REGISTRY}
}
...
deploy_to_kubernetes() {
    ...
    # gcr.io/bench-projects/wire-bot:tag
